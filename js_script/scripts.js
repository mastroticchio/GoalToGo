document.addEventListener("DOMContentLoaded", function () {
  ...
  if (fasceOrarieContainer) {
    ...
    if (campoInd)
      document.getElementById("dettaglioIndirizzo").innerText = campoInd;
    if (campoCitta)
      document.getElementById("dettaglioCitta").innerText = campoCitta;
    if (campoPrezzo)
      document.getElementById("dettaglioPrezzo").innerText =
        "€" + campoPrezzo + " / ora";
  ...
  cardDiv.innerHTML = `
    <div class="campo-card-header">
        ...
        <button class="elimina-campo-btn" data-id="${campo.ID}">🗑 Elimina</button>
    </div>
...
// blocca il click della card quando si clicca elimina
cardDiv.querySelector('.elimina-campo-btn').addEventListener('click', function (e) {
    e.stopPropagation(); // evita che apra pagina_gestione_campo.html

    const idCampo = this.dataset.id;

    Swal.fire({
        title: 'Sei sicuro?',
        text: 'Il campo e tutti i suoi orari verranno eliminati definitivamente.',
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            fetch('/GoalToGo/api/api_elimina_campo.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ id: idCampo })
            })
            .then(res => res.json())
            .then(data => {
                if (data.status === 'success') {
                    Swal.fire({
                        title: 'Eliminato!',
                        text: 'Il campo è stato rimosso.',
                        icon: 'success',
                        timer: 1500,
                        showConfirmButton: false
                    }).then(() => {
                        cardDiv.remove(); // rimuove la card dalla pagina senza ricaricare
                    });
                } else {
                    Swal.fire({ title: 'Errore', text: data.message, icon: 'error' });
                }
          ...
          listaCampiContainer.innerHTML =
            "<p>Errore nel caricamento dei campi</p>";
        }
      })
      .catch((err) => {
        console.error("Errore fetch campi:", err);
        listaCampiContainer.innerHTML =
          "<p>Errore di comunicazione col server</p>";
      });
  }
...
function aggiungiGiocatoreClub() {
  let giocatori_club =
    parseInt(document.getElementById("club-count").textContent) || 1;
